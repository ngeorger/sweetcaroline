...
jQuery( function( $ ) {
	var wcPaoInitAddonTotals = {
		...
		init: function( cart ) {
			...
			$cart
				...
				.on( 'keyup change', '.blv-cust-p-addon input, .blv-cust-p-addon textarea', function() {
					if ( $( this ).attr( 'maxlength' ) > 0 ) {

						var value     = $( this ).val();
						var remaining = $( this ).attr( 'maxlength' ) - value.length;

						$( this ).next( '.blv-cust-p-addon-chars-remaining' ).find( 'span' ).text( remaining );
					}
				} )

				.on( 'keyup', '.blv-cust-p-addon-custom-textarea, .blv-cust-p-addon-custom-text, .blv-cust-p-addon-custom-price', function() {
					$( this ).trigger( 'woocommerce-product-addons-update' );
				} )

				.on( 'change', '.blv-cust-p-addon input, .blv-cust-p-addon textarea, .blv-cust-p-addon select, input.qty', function() {
					$( this ).trigger( 'woocommerce-product-addons-update' );
				} )
				...
				.on( 'woocommerce-product-addons-update', function() {
					...
					$cart.find( '.blv-cust-p-addon-field' ).each( function() {
						var $addon                 = $( this ),
							parentContainer        = $addon.parents( '.blv-cust-p-addon' ),
							name                   = parentContainer.find( '.blv-cust-p-addon-name' ).length ? parentContainer.find( '.blv-cust-p-addon-name' ).data( 'addon-name' ) : '',
							value_label            = '',
							addon_cost             = 0,
							addon_cost_raw         = 0,
							...
							addon_data             = {},
							has_per_person_pricing = parentContainer.find( '.blv-cust-p-addon-name' ).length ? parentContainer.find( '.blv-cust-p-addon-name' ).data( 'has-per-person-pricing' ) : false,
							has_per_block_pricing  = parentContainer.find( '.blv-cust-p-addon-name' ).length ? parentContainer.find( '.blv-cust-p-addon-name' ).data( 'has-per-block-pricing' ) : false;

						if ( $addon.is( '.blv-cust-p-addon-custom-price' ) ) {
							if ( ! $addon.val() ) {
								return;
							}
							...
							price_type = 'flat_fee';
						} else if ( $addon.is( '.blv-cust-p-addon-input-multiplier' ) ) {
							if ( isNaN( $addon.val() ) || '' === $addon.val() ) { // Number inputs return blank when invalid
								$addon.val( '' );
								$addon.closest( 'p' ).find( '.blv-cust-p-addon-alert' ).show();
							} else {
								if ( '' !== $addon.val() ) {
									$addon.val( Math.ceil( $addon.val() ) );
								}

								$addon.closest( 'p' ).find( '.blv-cust-p-addon-alert' ).hide();
							}
						...
						} else if ( $addon.is( '.blv-cust-p-addon-checkbox, .blv-cust-p-addon-radio' ) ) {
							if ( ! $addon.is( ':checked' ) ) {
								return;
							}
						...
						} else if ( $addon.is( '.blv-cust-p-addon-image-swatch-select, .blv-cust-p-addon-select' ) ) {
						...
						if ( 'undefined' !== typeof value_label ) {
							if ( 'number' === typeof value_label || value_label.length ) {
								...
								if ( $addon.is( '.blv-cust-p-addon-file-upload' ) ) {
					...
					if ( qty ) {
						...
						if ( formatted_sub_total && '1' == $totals.data( 'show-sub-total' ) ) {
							var productName  = $cart.find( '.blv-cust-p-addon-container' ).data( 'product-name' ),
								productPrice = wcPaoInitAddonTotals.formatMoney( product_total_price );

							// If it is a bookable product.
							if ( $( '.wc-bookings-booking-form' ).length ) {
								var html = '<div class="product-addon-totals"><ul><li><div class="blv-cust-p-col1"><strong>' + productName + '</strong></div><div class="blv-cust-p-col2"><strong><span class="amount">' + productPrice + '</span></strong></div></li>';
							} else {
								var html = '<div class="product-addon-totals"><ul><li><div class="blv-cust-p-col1"><strong>' + qty + woocommerce_addons_params.quantity_symbol + productName + '</strong></div><div class="blv-cust-p-col2"><strong><span class="amount">' + productPrice + '</span></strong></div></li>';
							}
							...
							if ( addons.length ) {
								$.each( addons, function( i, addon ) {
									if ( 'quantity_based' === addon.price_type ) {
										...
										html = html + '<li class="blv-cust-p-row-quantity-based"><div class="blv-cust-p-col1">' + addon.name + '</div><div class="blv-cust-p-col2"><span class="amount">' + formattedValue + '</span></div></li>';
									}
								} );
								$.each( addons, function( i, addon ) {
									if ( 'quantity_based' !== addon.price_type ) {
										...
										html = html + '<li><div class="blv-cust-p-col1"><strong>' + addon.name + '</strong></div><div class="blv-cust-p-col2">' + formattedValue + '</div></li>';
									}
								} );
							}
							...
							if ( ! woocommerce_addons_params.price_display_suffix || ! woocommerce_addons_params.tax_enabled ) {
								html = html + '<li class="blv-cust-p-subtotal-line"><p class="price">' + sub_total_string + ' <span class="amount">' + formatted_sub_total + '</span></p></li></ul></div>';
								$totals.html( html );
							...
							if ( false === ( woocommerce_addons_params.price_display_suffix.indexOf( '{price_including_tax}' ) > -1 ) && false === ( woocommerce_addons_params.price_display_suffix.indexOf( '{price_excluding_tax}' ) > -1 ) ) {
								html = html + '<li class="blv-cust-p-subtotal-line"><strong>' + sub_total_string + ' <span class="amount">' + formatted_sub_total + '</span> ' + woocommerce_addons_params.price_display_suffix + '</strong></li></ul></div>';
								$totals.html( html );
								$cart.trigger( 'updated_addons' );
								return;
							}
							...
							$.ajax( {
								...
								success: function( result ) {
									if ( result.result == 'SUCCESS' ) {
										...
										html                 = html + '<li class="blv-cust-p-subtotal-line"><p class="price">' + sub_total_string + ' <span class="amount">' + formatted_sub_total + '</span> ' + price_display_suffix + ' </p></li></ul></div>';
										$totals.html( html );
										$cart.trigger( 'updated_addons' );
									} else {
										html = html + '<li class="blv-cust-p-subtotal-line"><p class="price">' + sub_total_string + ' <span class="amount">' + formatted_sub_total + '</span></p></li></ul></div>';
				...
				.on( 'click touchstart', '.blv-cust-p-addon-image-swatch', function( e ) {
					e.preventDefault();

					var selectedValue = $( this ).data( 'value' ),
						parent = $( this ).parents( '.blv-cust-p-addon-wrap' ),
						label = $.parseHTML( $( this ).data( 'price' ) );

					// Clear selected swatch label.
					parent.prev( 'label' ).find( '.blv-cust-p-addon-image-swatch-price' ).remove();

					// Clear all selected.
					parent.find( '.blv-cust-p-addon-image-swatch' ).removeClass( 'selected' );

					// Select this swatch.
					$( this ).addClass( 'selected' );

					// Set the value in hidden select field.
					parent.find( '.blv-cust-p-addon-image-swatch-select' ).val( selectedValue );
			...
			$cart.find( ' .blv-cust-p-addon-custom-text, .blv-cust-p-addon-custom-textarea' ).each( function() {
				if ( $( this ).attr( 'maxlength' ) > 0 ) {
					$( this ).after( '<small class="blv-cust-p-addon-chars-remaining"><span>' + $( this ).attr( 'maxlength' ) + '</span> ' + woocommerce_addons_params.i18n_remaining + '</small>' );
				}
			} );
			...
			$cart.find( '.blv-cust-p-addon-custom, .blv-cust-p-addon-custom-textarea, .blv-cust-p-addon input, .blv-cust-p-addon textarea, .blv-cust-p-addon select, input.qty, .variations select' ).change();

			$( '.blv-cust-p-addon-image-swatch' ).tipTip( { delay: 200 } );
		},
	};
	...
	$( 'body' ).find( '.blv-cust-p-addon-checkbox-group-required' ).each( function() {
		var checkboxesGroup = this;
		$( this ).find( '.blv-cust-p-addon-checkbox' ).each( function () {
			...
			$( this ).bind( 'invalid', function ( e ) {
				...
				$( checkboxesGroup ).addClass( "blv-cust-p-addon-checkbox-required-error" );
				e.preventDefault();
			} );
		})
		 ...
		 */
		$( this ).find( '.blv-cust-p-addon-checkbox' ).change( function () {
			// If user is making actions always remove error class.
			$( checkboxesGroup ).removeClass( "blv-cust-p-addon-checkbox-required-error" );
