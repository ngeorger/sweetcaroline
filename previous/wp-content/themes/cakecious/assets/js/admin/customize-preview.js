...
(function( $ ) {
	...
	var cakeciousCustomizer = {

		/**
		 * PostMessage: Output CSS.
		 */
		postMessage_css : function( key, rules, newValue ) {
			var styleID = 'cakecious-customize-preview-css-' + key,
			    $style = $( '#' + styleID ),
			    css = '',
			    cssArray = {};
		...
		postMessage_font : function( key, rules, newValue ) {
			...
			if ( -1 < newValue.indexOf( '|' ) ) {
				...
				if ( 2 === chunks.length && undefined !== cakeciousCustomizerPreviewData.fonts[ chunks[0] ] ) {
					fontSource = chunks[0],
					fontName = chunks[1],
					fontStack = cakeciousCustomizerPreviewData.fonts[ fontSource ][ fontName ] || '';
				}
			}

			// Change CSS value.
			cakeciousCustomizer.postMessage_css( key, rules, fontStack );

			// If value is from Google Fonts, update the embed link.
			if ( 'google_fonts' === fontSource ) {
				// Get the <link> tag.		
				var googleFontsLinkID = 'cakecious-customize-preview-google-fonts-css-' + key,
	...
	if ( undefined !== cakeciousCustomizerPreviewData ) {
		if ( undefined !== cakeciousCustomizerPreviewData.postMessages ) {
			_.each( cakeciousCustomizerPreviewData.postMessages, function( postMessages, key ) {
				...
				wp.customize( key, function( setting ) {
					setting.bind(function( value ) {
						_.each( sortedPostMessages, function( rule, type ) {
							...
							cakeciousCustomizer[ functionName ]( key, rule, value );
						});
					});
				});
			});
