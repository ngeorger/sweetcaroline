(function() {
	...
	window.cakecious = {
		...
		initDropdownMenuReposition: function() {
			var anchorSide = window.cakeciousHelper.isRTL() ? 'left' : 'right';

			var calculateSubMenuEdge = function() {

				var $submenus = Array.prototype.slice.call( document.querySelectorAll( '.cakecious-header-section .menu > * > .sub-menu' ) );
				$submenus.forEach(function( $submenu ) {
					var $section = $submenu.closest( '.cakecious-header-section' ),
					    $menuItem = $submenu.parentElement,
					    $wrapper = $menuItem.closest( '.cakecious-wrapper' ),
					    $container = $wrapper;

					// Full width mega menu, use section as the container.
					if ( $menuItem.classList.contains( 'cakecious-mega-menu' ) && $menuItem.classList.contains( 'cakecious-mega-menu-full-width' ) ) {
						$container = $section;
					}
					// Contained section, use section inner as the container.
					else if ( $section.classList.contains( 'cakecious-section-contained' ) ) {
						$container = $section.querySelector( '.cakecious-section-inner' );
					} 

					// Reset inline styling.
					$submenu.classList.remove( 'cakecious-sub-menu-edge' );
					...
					var containerEdge = $container.getBoundingClientRect()[ anchorSide ],
						...
						isSubmenuOverflow = window.cakeciousHelper.isRTL() ? submenuEdge < containerEdge : submenuEdge > containerEdge;

					// Apply class and left position.
					if ( isSubmenuOverflow ) {
						$submenu.classList.add( 'cakecious-sub-menu-edge' );
						$submenu.style[ anchorSide ] = ( window.cakeciousHelper.isRTL() ? $container.getBoundingClientRect()[ anchorSide ] - $wrapper.getBoundingClientRect()[ anchorSide ] : $wrapper.getBoundingClientRect()[ anchorSide ] - $container.getBoundingClientRect()[ anchorSide ] ) + 'px';
					}

					if ( $menuItem.classList.contains( 'cakecious-mega-menu' ) && $menuItem.classList.contains( 'cakecious-mega-menu-full-width' ) ) {
						var maxContentWidth = $section.classList.contains( 'cakecious-section-contained' ) ? $menuItem.closest( '.cakecious-section-inner' ).offsetWidth : $menuItem.closest( '.cakecious-wrapper' ).offsetWidth,
					...
					$subsubmenus.forEach(function( $subsubmenu ) {
						var subsubmenuEdge = $subsubmenu.getBoundingClientRect().left + ( window.cakeciousHelper.isRTL() ? 0 : $subsubmenu.getBoundingClientRect().width ),
							isSubsubmenuOverflow = window.cakeciousHelper.isRTL() ? subsubmenuEdge < containerEdge : subsubmenuEdge > containerEdge;

						// Reset inline styling.
						$subsubmenu.classList.remove( 'cakecious-sub-menu-right' );

						// Apply class and left position.
						if ( isSubsubmenuOverflow ) {
							$subsubmenu.classList.add( 'cakecious-sub-menu-right' );
						}
		...
		initMenuAccessibility: function() {
			...
			var handleMenuFocusUsingKeyboard = function( e ) {
				var $this = e.target,
				    $menu = $this.closest( '.cakecious-hover-menu' ),
				    $current = this;
			...
			var $menuLinks = Array.prototype.slice.call( document.querySelectorAll( '.cakecious-hover-menu .menu-item > a' ) );
			...
			var handleMenuNavigationUsingKeyboard = function( e ) {
				...
				var $this = e.target.closest( '.cakecious-hover-menu .menu-item > a' );
				if ( ! $this ) return;
		...
		initClickToggleDropdownMenu: function() {
			...
			var handleSubMenuToggle = function( e ) {
				// Check target element.
				var $this = e.target.closest( '.cakecious-header-section .cakecious-toggle-menu .cakecious-sub-menu-toggle' );
				if ( ! $this ) return;
			...
			var handleSubMenuClose = function( e ) {
				...
				if ( ! e.target.closest( '.cakecious-header-section .cakecious-toggle-menu' ) ) {
					var $header = document.getElementById( 'masthead' ),
					    $focusedMenuItems;

					if ( $header ) {
						var $focusedMenuItems = Array.prototype.slice.call( $header.querySelectorAll( '.cakecious-toggle-menu .menu-item.focus' ) );
						$focusedMenuItems.forEach(function( $focusedMenuItem ) {
							$focusedMenuItem.classList.remove( 'focus' );
		...
		initAccordionMenu: function() {
			...
			var handleAccordionMenuToggle = function( e ) {
				// Check target element.
				var $this = e.target.closest( '.cakecious-header-section-vertical .cakecious-toggle-menu .cakecious-sub-menu-toggle' );
				if ( ! $this ) return;
				...
				if ( $menuItem.classList.contains( 'focus' ) ) {
					window.cakeciousHelper.slideUp( $subMenu );
					$menuItem.classList.remove( 'focus' );
					...
					$insideMenuItems.forEach(function( $insideMenuItem ) {
						window.cakeciousHelper.slideUp( $insideMenuItem.querySelector( '.sub-menu' ) );
				...
				// Menu item doesn't have "focus" class yet, so collapses all focused siblings and focuses this menu item.
					...
					$siblingMenuItems.forEach(function( $siblingMenuItem ) {
						window.cakeciousHelper.slideUp( $siblingMenuItem.querySelector( '.sub-menu' ) );
						$siblingMenuItem.classList.remove( 'focus' );
					});

					window.cakeciousHelper.slideDown( $subMenu );
					$menuItem.classList.add( 'focus' );
				}
			}
			...
			var handleAccordionMenuEmptyHashLink = function( e ) {
				// Check target element.
				var $this = e.target.closest( '.cakecious-header-section-vertical .cakecious-toggle-menu .menu-item-has-children > .cakecious-menu-item-link[href="#"]' );
				if ( ! $this ) return;

				e.preventDefault();

				var $menuItem = $this.parentElement,
				    $toggle = $menuItem.querySelector( '.cakecious-sub-menu-toggle' );
		...
		initGlobalPopup: function() {
			...
			var deactivatePopup = function( device ) {
				var $activePopups = Array.prototype.slice.call( document.querySelectorAll( '.cakecious-popup-active' + ( undefined !== device ? '.cakecious-hide-on-' + device : '' ) ) );

				$activePopups.forEach(function( $activePopup ) {
					// Deactivate popup.
					$clickedToggle.classList.remove( 'cakecious-popup-toggle-active' );
					$clickedToggle.setAttribute( 'aria-expanded', false );
					$activePopup.classList.remove( 'cakecious-popup-active' );
					document.body.classList.remove( 'cakecious-has-popup-active' );
			...
			var handlePopupToggle = function( e ) {
				// Check target element.
				var $this = e.target.closest( '.cakecious-popup-toggle' );
				if ( ! $this ) return;
				...
				if ( $target.classList.contains( 'cakecious-popup-active' ) ) {
					deactivatePopup();
				} else {
					// Activate popup.
					$this.classList.add( 'cakecious-popup-toggle-active' );
					$this.setAttribute( 'aria-expanded', true );
					$target.classList.add( 'cakecious-popup-active' );
					document.body.classList.add( 'cakecious-has-popup-active' );
			...
			// Close popup when any of ".cakecious-popup-close" element is clicked.
			var handlePopupClose = function( e ) {
				// Check target element.
				if ( ! e.target.closest( '.cakecious-popup-close' ) ) return;
			...
			var handlePopupEscape = function( e ) {
				...
				if ( document.body.classList.contains( 'cakecious-has-popup-active' ) && 27 === key ) {
					deactivatePopup();
				}
			}
			...
			var handleResponsiveVisibility = function( e ) {
				if ( document.body.classList.contains( 'cakecious-has-popup-active' ) ) {
					...
					if ( cakeciousConfig.breakpoints.mobile <= window.innerWidth ) {
						device = 'tablet';
					}
			...
			var handleHashLinkInsidePopup = function( e ) {
				// Check target element.
				if ( ! e.target.closest( '.cakecious-popup a' ) ) return;
		...
		initScrollToTop: function() {
			var $scrollToTop = document.querySelector( '.cakecious-scroll-to-top' );

			if ( $scrollToTop ) {
				var handleScrollToTop = function( e ) {
					// Check target element.
					if ( ! e.target.closest( '.cakecious-scroll-to-top' ) ) return;

					e.preventDefault();

					var $link = e.target.closest( '.cakecious-scroll-to-top' ),
				...
				if ( $scrollToTop.classList.contains( 'cakecious-scroll-to-top-display-sticky' ) ) {
					var checkStickyOffset = function() {
						if ( window.pageYOffset > 0.5 * window.innerHeight ) {
							$scrollToTop.classList.add( 'cakecious-sticky' );
						} else {
							$scrollToTop.classList.remove( 'cakecious-sticky' );
						}
					}
		 ...
		 */
		initAll: function() {
			window.cakecious.initKeyboardAndMouseFocus();
			window.cakecious.initDropdownMenuReposition();
			window.cakecious.initMenuAccessibility();
			window.cakecious.initClickToggleDropdownMenu();
			window.cakecious.initDoubleTapMobileMenu();
			window.cakecious.initAccordionMenu();
			window.cakecious.initGlobalPopup();
			window.cakecious.initScrollToTop();
		},
	}
